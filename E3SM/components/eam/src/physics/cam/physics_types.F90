...
!-------------------------------------------------------------------------------
  type physics_state
     integer                                     :: &
          ...
          nmem=16   ! Convective memory (hidden NN variable defined at every level)
#else
          ncol=0                 ! --             -- sum of nsubcol for all ngrdcols - number of active columns
#endif
...
! allocate the individual state components
  ...
  allocate(state%conv_mem(16, pver, psetcols), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%conv_mem')

  allocate(state%eps(144, psetcols, pver), stat=ierr)
 ! allocate(state%eps(144, psetcols, pver, 2), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%eps')
...
#if defined(MMF_ML_TRAINING) || defined(MMF_NN_EMULATOR)
  deallocate(state%conv_mem, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: deallocation error for state%conv_mem')

  deallocate(state%eps, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: deallocation error for state%eps')
